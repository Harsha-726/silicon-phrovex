import { addDays, assessmentIdempotencyKey, dateAt, expandRecurringTask, formatDate, formatLongDate, formatTime, isOverdue, makeTask, planStudySessions, rankRecommendations, recurrenceFromText, seedState, taskSort, toDateKey, INTENTS } from './core.js';
...
function persistProfile() { repository?.saveProfile(state.profile).catch(() => showToast('Could not sync preferences. They are saved locally.')); }
...
function renderSettings() { return `<div class="settings-layout"><section class="settings-section"><div class="settings-title"><h2>Study preferences</h2><p>Silico uses these preferences to place realistic study sessions around your commitments.</p></div><label>Default session length<select id="session-length"><option value="30" ${state.profile.sessionLength === 30 ? 'selected' : ''}>30 minutes</option><option value="45" ${state.profile.sessionLength === 45 ? 'selected' : ''}>45 minutes</option><option value="60" ${state.profile.sessionLength === 60 ? 'selected' : ''}>1 hour</option></select></label><label>Sessions per assessment<input id="sessions-per-assessment" type="number" min="1" max="5" value="${state.profile.sessionsPerAssessment}"/></label><div class="settings-row"><label>Study window starts<input id="preferred-start" type="time" value="${state.profile.preferredStart}"/></label><label>Latest study time<input id="latest-study" type="time" value="${state.profile.latestStudyTime}"/></label></div><button class="primary-button" data-action="save-settings">Save preferences</button></section><section class="settings-section"><div class="settings-title"><h2>Class allocation</h2><p>Give each class its own rhythm. Silico uses these values when planning around an assessment.</p></div>${state.classes.map(name => { const preference = state.profile.classPreferences?.[name] || {}; return `<div class="settings-row class-preference"><strong>${escapeHtml(name)}</strong><label>Sessions/week<input class="class-pref-sessions" data-class="${escapeHtml(name)}" type="number" min="0" max="14" value="${preference.sessionsPerWeek || 2}"/></label><label>Session length<select class="class-pref-length" data-class="${escapeHtml(name)}"><option value="30" ${preference.sessionLength === 30 ? 'selected' : ''}>30 min</option><option value="45" ${(!preference.sessionLength || preference.sessionLength === 45) ? 'selected' : ''}>45 min</option><option value="60" ${preference.sessionLength === 60 ? 'selected' : ''}>1 hour</option></select></label></div>`; }).join('')}</section><section class="settings-section"><div class="settings-title"><h2>Scheduling boundaries</h2><p>Silico always avoids school hours (8:00 AM–4:00 PM) and never schedules in the past.</p></div><div class="boundary-row"><span class="boundary-icon">⌁</span><div><strong>School hours</strong><small>8:00 AM — 4:00 PM · Protected</small></div><span class="status-pill">Default</span></div><div class="boundary-row"><span class="boundary-icon">◷</span><div><strong>Timezone</strong><small>${Intl.DateTimeFormat().resolvedOptions().timeZone}</small></div></div></section><section class="settings-section danger-zone"><div class="settings-title"><h2>Workspace data</h2><p>Reset the local demo workspace or remove every task and derived occurrence.</p></div><div class="header-actions"><button class="danger-button" data-action="delete-all">Delete all tasks</button><button class="secondary-button" data-action="reset-data">Reset demo</button></div></section></div>`; }
...
function renderOnboarding() {
  const classFields = state.classes.map(name => `<div class="settings-row class-preference"><strong>${escapeHtml(name)}</strong><label>Sessions/week<input class="onboarding-class-sessions" data-class="${escapeHtml(name)}" type="number" min="0" max="14" value="${state.profile.classPreferences?.[name]?.sessionsPerWeek || 2}"/></label><label>Length<select class="onboarding-class-length" data-class="${escapeHtml(name)}"><option value="30">30 min</option><option value="45" selected>45 min</option><option value="60">1 hour</option></select></label></div>`).join('');
  document.querySelector('#app').innerHTML = `<main class="onboarding-shell"><div class="onboarding-card"><div class="brand"><span class="brand-mark">S</span><span>Silico</span></div><div class="eyebrow">A few preferences first</div><h1>Make study time fit your life.</h1><p class="onboarding-copy">Silico uses these defaults to place realistic study sessions around your commitments. You can change them anytime.</p><form id="onboarding-form"><label>How long should a typical study session be?<select id="onboarding-length"><option value="30">30 minutes</option><option value="45" selected>45 minutes</option><option value="60">1 hour</option></select></label><label>How many sessions per week should Silico aim for?<input id="onboarding-weekly" type="number" min="0" max="14" value="2"/></label><div class="settings-row"><label>Study window starts<input id="onboarding-start" type="time" value="16:30"/></label><label>Latest study time<input id="onboarding-latest" type="time" value="21:00"/></label></div><fieldset><legend>Class allocation</legend>${classFields}</fieldset><fieldset><legend>Methods you enjoy</legend><label class="check-label"><input type="checkbox" value="Review notes" checked/> Review notes</label><label class="check-label"><input type="checkbox" value="Practice problems"/> Practice problems</label><label class="check-label"><input type="checkbox" value="Flashcards"/> Flashcards</label><label class="check-label"><input type="checkbox" value="Reading"/> Reading</label></fieldset><button class="primary-button" data-action="complete-onboarding">Continue to Silico <span>→</span></button></form></div></main>`;
  document.querySelector('#onboarding-form')?.addEventListener('submit', event => event.preventDefault());
  bindEvents();
}

function renderTaskDrawer() { const task = getTaskById(selectedTaskId); if (!task) return ''; const recurrenceText = task.recurrence ? `every ${task.recurrence.frequency === 'daily' ? 'day' : task.recurrence.frequency === 'monthly' ? 'month' : 'week'}` : ''; return `<div class="drawer-backdrop" data-action="close-drawer"></div><aside class="task-drawer"><div class="drawer-header"><span>Task details</span><button class="icon-button" data-action="close-drawer">×</button></div><input class="drawer-title" id="drawer-title" value="${escapeHtml(task.title)}"/><div class="drawer-fields"><label>Due date<input id="drawer-date" type="date" value="${task.dueDate || ''}"/></label><label>Time<input id="drawer-time" type="time" value="${task.dueTime || ''}"/></label><label>Duration<select id="drawer-duration"><option ${task.duration === 30 ? 'selected' : ''}>30</option><option ${task.duration === 45 ? 'selected' : ''}>45</option><option ${task.duration === 60 ? 'selected' : ''}>60</option><option ${task.duration === 90 ? 'selected' : ''}>90</option></select></label><label>Priority<select id="drawer-priority"><option value="1" ${task.priority === 1 ? 'selected' : ''}>Normal</option><option value="2" ${task.priority === 2 ? 'selected' : ''}>High</option><option value="3" ${task.priority === 3 ? 'selected' : ''}>Urgent</option><option value="4" ${task.priority === 4 ? 'selected' : ''}>Critical</option></select></label><label>Class<input id="drawer-class" value="${escapeHtml(task.className || '')}" placeholder="e.g. Biology"/></label><label>Project<input id="drawer-project" value="${escapeHtml(task.project || '')}" placeholder="e.g. Homework"/></label><label>Recurrence<input id="drawer-recurrence" value="${escapeHtml(recurrenceText)}" placeholder="e.g. every Monday"/></label></div><textarea id="drawer-description" placeholder="Add a description…">${escapeHtml(task.description || '')}</textarea><div class="drawer-footer"><button class="danger-button" data-action="delete-task" data-id="${task.id}">Delete</button><button class="primary-button" data-action="save-task" data-id="${task.id}">Save</button></div></aside>`; }
...
function handleAction(action, id) {
  ...
  if (action === 'toggle-task') { const task = getTaskById(id); if (task?.occurrenceKey) { state.occurrenceCompletions ||= {}; const completed = task.status !== 'completed'; if (completed) state.occurrenceCompletions[task.occurrenceKey] = new Date().toISOString(); else delete state.occurrenceCompletions[task.occurrenceKey]; saveState(); repository?.setOccurrence(task.recurrenceSourceId, task.dueDate, completed).catch(() => showToast('Could not sync this occurrence. It is saved locally.')); render(); return; } if (task) { task.status = task.status === 'completed' ? 'open' : 'completed'; task.completedAt = task.status === 'completed' ? new Date().toISOString() : null; task.updatedAt = new Date().toISOString(); saveState(); repository?.update(task).catch(() => showToast('Could not sync this task. It is saved locally.')); render(); } return; }
  ...
  if (action === 'save-settings') { state.profile.sessionLength = Number(document.querySelector('#session-length').value); state.profile.sessionsPerAssessment = Number(document.querySelector('#sessions-per-assessment').value); state.profile.preferredStart = document.querySelector('#preferred-start').value; state.profile.latestStudyTime = document.querySelector('#latest-study').value; state.profile.classPreferences ||= {}; document.querySelectorAll('.class-pref-sessions').forEach(input => { state.profile.classPreferences[input.dataset.class] ||= {}; state.profile.classPreferences[input.dataset.class].sessionsPerWeek = Number(input.value); }); document.querySelectorAll('.class-pref-length').forEach(input => { state.profile.classPreferences[input.dataset.class] ||= {}; state.profile.classPreferences[input.dataset.class].sessionLength = Number(input.value); }); saveState(); persistProfile(); render(); return; }
  if (action === 'complete-onboarding') { const classPreferences = {}; document.querySelectorAll('.onboarding-class-sessions').forEach(input => { classPreferences[input.dataset.class] = { sessionsPerWeek: Number(input.value), sessionLength: Number(document.querySelector(`.onboarding-class-length[data-class="${CSS.escape(input.dataset.class)}"]`).value) }; }); state.profile = { ...state.profile, onboardingComplete: true, sessionLength: Number(document.querySelector('#onboarding-length').value), sessionsPerWeek: Number(document.querySelector('#onboarding-weekly').value), sessionsPerAssessment: Math.max(1, Math.min(5, Number(document.querySelector('#onboarding-weekly').value) || 2)), preferredStart: document.querySelector('#onboarding-start').value, latestStudyTime: document.querySelector('#onboarding-latest').value, methods: [...document.querySelectorAll('#onboarding-form input[type="checkbox"]:checked')].map(input => input.value), classPreferences }; saveState(); persistProfile(); render(); return; }
...
async function bootstrap() {
  ...
  state = loadState();
  if (repository) { try { state.tasks = await repository.load(); const remoteProfile = await repository.loadProfile(); if (remoteProfile?.profile?.settings && typeof remoteProfile.profile.settings === 'object') state.profile = { ...state.profile, ...remoteProfile.profile.settings, onboardingComplete: Boolean(remoteProfile.profile.onboarding_complete), classPreferences: { ...state.profile.classPreferences, ...(remoteProfile.profile.settings.classPreferences || {}) } }; saveState(); } catch { /* local fallback remains available until server secrets are configured */ } }
