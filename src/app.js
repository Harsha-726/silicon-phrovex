...
function handleAction(action, id) {
  ...
  if (action === 'save-settings') { state.profile.sessionLength = Number(document.querySelector('#session-length').value); state.profile.sessionsPerAssessment = Number(document.querySelector('#sessions-per-assessment').value); state.profile.preferredStart = document.querySelector('#preferred-start').value; state.profile.latestStudyTime = document.querySelector('#latest-study').value; state.profile.classPreferences ||= {}; document.querySelectorAll('.class-pref-sessions').forEach(input => { state.profile.classPreferences[input.dataset.class] ||= defaultClassPreference(); state.profile.classPreferences[input.dataset.class].sessionsPerWeek = Math.max(0, Math.min(14, Number(input.value) || 0)); }); document.querySelectorAll('.class-pref-length').forEach(input => { state.profile.classPreferences[input.dataset.class] ||= defaultClassPreference(); state.profile.classPreferences[input.dataset.class].sessionLength = Number(input.value); }); saveState(); persistProfile(); render(); return; }
  if (action === 'complete-onboarding') { collectOnboardingClassPreferences(); state.profile = { ...state.profile, onboardingComplete: true, sessionLength: Number(document.querySelector('#onboarding-length').value), sessionsPerWeek: Math.max(0, Math.min(14, Number(document.querySelector('#onboarding-weekly').value) || 0)), sessionsPerAssessment: Math.max(1, Math.min(5, Number(document.querySelector('#onboarding-weekly').value) || 2)), preferredStart: document.querySelector('#onboarding-start').value, latestStudyTime: document.querySelector('#onboarding-latest').value, methods: [...document.querySelectorAll('#onboarding-form input[type="checkbox"]:checked')].map(input => input.value) }; saveState(); persistProfile(); render(); return; }
