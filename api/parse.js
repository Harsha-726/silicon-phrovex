...
export default async function handler(request, response) {
  ...
  try {
    const upstream = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'groq/compound-mini', temperature: 0, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: 'Return only a JSON intent command. Allowed intents: CREATE_TASK, CREATE_ASSESSMENT, CREATE_RECURRING_TASK, EDIT_TASK, DELETE_TASK, COMPLETE_TASK, RESCHEDULE_TASK, QUERY_TODAY, QUERY_UPCOMING, QUERY_FREE_TIME, QUERY_RECOMMENDATION, STUDY_PLANNING, GENERAL_QUERY. Use CREATE_ASSESSMENT for a test, exam, quiz, or assessment with a due date. Use STUDY_PLANNING only for an explicit request to plan studying. Never include database IDs, user IDs, authorization decisions, or mutations.' }, { role: 'user', content: input }], max_tokens: 300 })
    });
